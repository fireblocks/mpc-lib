...
public:
    ...
    public:
        ...
        // This function set the data at index, in case index is larger than the array size the function should throw exception
    ...
    uint64_t offline_mta_response(const std::string& request_id, const std::map<uint64_t, std::vector<cmp_mta_request>>& requests, uint32_t version, cmp_mta_responses& response);
