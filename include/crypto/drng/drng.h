...
COSIGNER_EXPORT drng_status drng_new(const uint8_t *seed, uint32_t seed_len, drng_t **rng);
COSIGNER_EXPORT drng_status drng_read_deterministic_rand(drng_t *rng, uint8_t *rand, uint32_t length_in_bytes);
COSIGNER_EXPORT void drng_free(drng_t *rng);
