...
#include "../../common/crypto/paillier/paillier_internal.h"
#include "../../common/crypto/commitments/ring_pedersen_internal.h"
...
struct Sha256Hasher
{
    Sha256Hasher(SHA256_CTX& ctx) : _ctx(ctx) {}
    void hash_vector_with_size(const byte_vector_t& vec)
    {
        const uint32_t size = vec.size();
        SHA256_Update(&_ctx, &size, sizeof(uint32_t));
        SHA256_Update(&_ctx, vec.data(), vec.size());
    }

    void hash_bn(BIGNUM* bn, const size_t size, const char* name)
    {
        _vec.resize(size);
        if (BN_bn2binpad(bn, _vec.data(), _vec.size()) <= 0)
        {
            const uint32_t required_size  = BN_num_bytes(bn);
            LOG_ERROR("Error hashing %s- not enough buffer. Required %u, given %u", name, required_size, (uint32_t) _vec.size());
            throw_cosigner_exception(cosigner_exception::INVALID_PARAMETERS);
        }
        SHA256_Update(&_ctx, _vec.data(), size);
    }
...
static inline void generate_mta_range_zkp_extended_seed(const cmp_mta_message& response,
                                                        const mta_range_zkp& proof,
                                                        const std::vector<uint8_t>& aad,
                                                        const ring_pedersen_public_t* ring_pedersen_verifier,
                                                        const paillier_public_key_t* public_key_prover,
                                                        const paillier_public_key_t* public_key_verifier,
                                                        uint8_t *seed)
{
    SHA256_CTX ctx;
    const uint32_t verifier_ring_pedersen_n_size = BN_num_bytes(ring_pedersen_verifier->n);
    const uint32_t prover_paillier_pub_n_size = BN_num_bytes(public_key_prover->n);
    const uint32_t verifier_paillier_pub_n_size = BN_num_bytes(public_key_verifier->n);

    SHA256_Init(&ctx);
    SHA256_Update(&ctx, MTA_ZKP_SALT, sizeof(MTA_ZKP_SALT));

    Sha256Hasher hasher(ctx);
    ...
    hasher.hash_bn(proof.By, prover_paillier_pub_n_size * 2, "By");
    hasher.hash_bn(proof.E, verifier_ring_pedersen_n_size, "E");
    hasher.hash_bn(proof.F, verifier_ring_pedersen_n_size, "F");
    hasher.hash_bn(proof.S, verifier_ring_pedersen_n_size, "S");
    hasher.hash_bn(proof.T, verifier_ring_pedersen_n_size, "T");
...
static inline std::vector<uint8_t> serialize_mta_range_zkp(const mta_range_zkp& proof,
                                                           const ring_pedersen_public_t* ring_pedersen,
                                                           const paillier_private_key_t* private_key,
                                                           const paillier_public_key_t* public_key)
{
    ...
    *(uint32_t*)ptr = paillier_pub_n_size;
    ...
    if (BN_bn2binpad(proof.By, ptr, paillier_priv_n_size * 2) <= 0)
    {
        throw_cosigner_exception(cosigner_exception::INTERNAL_ERROR);
    }
    ptr += paillier_priv_n_size * 2;
    if (BN_bn2binpad(proof.E, ptr, ring_pedersen_n_size) <= 0)
    {
        throw_cosigner_exception(cosigner_exception::INTERNAL_ERROR);
    }
    ptr += ring_pedersen_n_size;
    if (BN_bn2binpad(proof.F, ptr, ring_pedersen_n_size) <= 0)
    {
        throw_cosigner_exception(cosigner_exception::INTERNAL_ERROR);
    }
    ptr += ring_pedersen_n_size;
    ...
    if (BN_bn2binpad(proof.z1, ptr, MTA_ZKP_EPSILON_SIZE + sizeof(elliptic_curve256_scalar_t) + 1) <= 0)
    {
        throw_cosigner_exception(cosigner_exception::INTERNAL_ERROR);
    }
    ptr += MTA_ZKP_EPSILON_SIZE + sizeof(elliptic_curve256_scalar_t) + 1;
    if (BN_bn2binpad(proof.z2, ptr, MTA_ZKP_EPSILON_SIZE + sizeof(elliptic_curve256_scalar_t) * BETA_HIDING_FACTOR + 1) <= 0)
    {
        throw_cosigner_exception(cosigner_exception::INTERNAL_ERROR);
    }
    ptr += MTA_ZKP_EPSILON_SIZE + sizeof(elliptic_curve256_scalar_t) * BETA_HIDING_FACTOR + 1;
    if (BN_bn2binpad(proof.z3, ptr, MTA_ZKP_EPSILON_SIZE + sizeof(elliptic_curve256_scalar_t) + BN_num_bytes(ring_pedersen->n) + 1) <= 0)
    {
        throw_cosigner_exception(cosigner_exception::INTERNAL_ERROR);
    }
    ptr += MTA_ZKP_EPSILON_SIZE + sizeof(elliptic_curve256_scalar_t) + BN_num_bytes(ring_pedersen->n) + 1;
    if (BN_bn2binpad(proof.z4, ptr, MTA_ZKP_EPSILON_SIZE + sizeof(elliptic_curve256_scalar_t) + BN_num_bytes(ring_pedersen->n) + 1) <= 0)
    {
        throw_cosigner_exception(cosigner_exception::INTERNAL_ERROR);
    }
    ptr += MTA_ZKP_EPSILON_SIZE + sizeof(elliptic_curve256_scalar_t) + BN_num_bytes(ring_pedersen->n) + 1;
    if (BN_bn2binpad(proof.w, ptr, paillier_pub_n_size) <= 0)
    {
        throw_cosigner_exception(cosigner_exception::INTERNAL_ERROR);
    }
    ptr += paillier_pub_n_size;
    if (BN_bn2binpad(proof.wy, ptr, paillier_priv_n_size) <= 0)
    {
        throw_cosigner_exception(cosigner_exception::INTERNAL_ERROR);
    }
...
static inline void deserialize_mta_range_zkp(std::vector<uint8_t> buff, const ring_pedersen_public_t* ring_pedersen, const paillier_private_key_t* private_key, const paillier_public_key_t* public_key, mta_range_zkp& proof)
{
    ...
    {
        LOG_ERROR("Invalid buffer size %lu, expected %u", buff.size(), exponent_zkpok_serialized_size(ring_pedersen, private_key, public_key));
        throw_cosigner_exception(cosigner_exception::INVALID_PARAMETERS);
    }
    const uint32_t ring_pedersen_n_size = BN_num_bytes(ring_pedersen->n);
    const uint32_t paillier_priv_n_size = BN_num_bytes(private_key->pub.n);
...
static std::vector<uint8_t> mta_range_generate_zkp(const elliptic_curve256_algebra_ctx_t* algebra,
                                                   const ring_pedersen_public_t* ring_pedersen,
                                                   const paillier_private_key_t* private_key,
                                                   const paillier_public_key_t* public_key,
                                                   const std::vector<uint8_t>& aad,
                                                   const BIGNUM* x,
                                                   const BIGNUM* y,
                                                   const BIGNUM* mta_request,
                                                   const BIGNUM* mta_response_r,
                                                   const paillier_ciphertext_t* commitment,
                                                   const uint32_t version,
                                                   const cmp_mta_message& response)
{
    ...
    if (BN_bn2binpad(tmp, alpha_bin, sizeof(elliptic_curve256_scalar_t)) <= 0) {
        LOG_ERROR("Failed to serialize alpha");
        throw_cosigner_exception(cosigner_exception::INTERNAL_ERROR);
    }
    ...
    {
        generate_mta_range_zkp_extended_seed(response, proof, aad, ring_pedersen, &private_key->pub, public_key, seed);
    }
    else
    {
        generate_mta_range_zkp_seed(response, proof, aad, seed);
    }
...
//this function prefills the common part and creates a map of proofs to fill the remaining part for each party individually.
                        ...
                        std::map<uint64_t, byte_vector_t>& G_proofs)            //output map all all parties (players) ids to "log" proof messages
{
    ...
    for (auto i = players.begin(); i != players.end(); ++i)
    {
        if (i->first == my_id)
            continue;
        uint32_t len = 0;
        range_proof_diffie_hellman_zkpok_generate(i->second.ring_pedersen.get(), paillier.get(), algebra, aad.data(), aad.size(), &k.data, &a.data, &b.data, ciphertext, /*use_extended_seed=*/0, NULL, 0, &len);
        auto& proof = proofs[i->first];
        proof.resize(len);
        auto status = range_proof_diffie_hellman_zkpok_generate(i->second.ring_pedersen.get(), paillier.get(), algebra, aad.data(), aad.size(), &k.data, &a.data, &b.data, ciphertext, /*use_extended_seed=*/0, proof.data(), proof.size(), &len);
        ...
        len = 0;
        range_proof_paillier_exponent_zkpok_generate(i->second.ring_pedersen.get(), paillier.get(), algebra, aad.data(), aad.size(), &gamma.data, commitment, /*use_extended_seed=*/0, NULL, 0, &len);
        auto& G_proof = G_proofs[i->first];
        G_proof.resize(len);
        status = range_proof_paillier_exponent_zkpok_generate(i->second.ring_pedersen.get(), paillier.get(), algebra, aad.data(), aad.size(), &gamma.data, commitment, /*use_extended_seed=*/0, G_proof.data(), G_proof.size(), &len);
        if (status != ZKP_SUCCESS)
        {
    ...
    mta.message.resize(BN_num_bytes(paillier->n2));
    assert(ciphertext->cipher_size == mta.message.size());
    if (BN_bn2binpad(ciphertext->ciphertext, mta.message.data(), mta.message.size()) <= 0)
    {
        LOG_ERROR("Failed to encode mta message");
        throw_cosigner_exception(cosigner_exception::INTERNAL_ERROR);
    }
    ciphertext_guard.reset();
    mta.commitment.resize(BN_num_bytes(paillier->n2));
    assert(commitment->cipher_size == mta.commitment.size());
    if (BN_bn2binpad(commitment->ciphertext, mta.commitment.data(), mta.commitment.size()) <= 0)
    {
        LOG_ERROR("Failed to encode mta commitment");
        throw_cosigner_exception(cosigner_exception::INTERNAL_ERROR);
    }
    return mta;
}
                                         ...
                                         cmp_mta_message& response)
{
    ...
    utils::byte_vector_cleaner beta_cleaner(beta);
    ...
    response.proof = mta_range_generate_zkp(algebra,
                                            ring_pedersen.get(),
                                            my_key.get(),
                                            paillier.get(),
                                            aad,
    ...
    if (BN_bn2binpad(y.get(), ret.data, sizeof(elliptic_curve256_scalar_t)) <= 0)
    {
        LOG_ERROR("Failed to serialize y");
        throw_cosigner_exception(cosigner_exception::INTERNAL_ERROR);
    }
    return ret;
}
...
elliptic_curve_scalar decrypt_mta_response(uint64_t other_id, const elliptic_curve256_algebra_ctx_t* algebra, byte_vector_t&& response, const std::shared_ptr<paillier_private_key_t>& my_key)
{
    ...
    elliptic_curve_scalar ret;
    if (BN_bn2binpad(alpha.get(), ret.data, sizeof(elliptic_curve256_scalar_t)) <= 0)
    {
        LOG_ERROR("Failed to serialize alpha");
        throw_cosigner_exception(cosigner_exception::INTERNAL_ERROR);
    }
...
base_response_verifier::base_response_verifier(const uint32_t version,
...
batch_response_verifier::batch_response_verifier(
        ...
        const std::shared_ptr<paillier_public_key_t>& paillier,
        const std::shared_ptr<ring_pedersen_private_t>& ring_pedersen) :
    base_response_verifier(version, other_id, algebra, aad, my_key, paillier, ring_pedersen)
{
    ...
    for (size_t i = 0; i < BATCH_STATISTICAL_SECURITY; i++)
    {
        ...
        if (!BN_one(_mta_ro[i]))
        {
            LOG_ERROR("Failed to set BN_one for _mta_ro");
            throw_cosigner_exception(cosigner_exception::NO_MEM);
        }
        if (!BN_one(_mta_B[i]))
        {
            LOG_ERROR("Failed to set BN_one for _mta_B");
            throw_cosigner_exception(cosigner_exception::NO_MEM);
        }
        if (!BN_one(_commitment_ro[i]))
        {
            LOG_ERROR("Failed to set BN_one for _commitment_ro");
            throw_cosigner_exception(cosigner_exception::NO_MEM);
        }
        if (!BN_one(_commitment_B[i]))
        {
            LOG_ERROR("Failed to set BN_one for _commitment_B");
            throw_cosigner_exception(cosigner_exception::NO_MEM);
        }
    }
    ...
    if (!BN_one(_pedersen_B))
    {
        LOG_ERROR("Failed to set BN_one for _pedersen_B");
        throw_cosigner_exception(cosigner_exception::NO_MEM);
    }
    ...
    const elliptic_curve_point& public_point)
{
    ...
    {
        generate_mta_range_zkp_extended_seed(response, proof, _aad, &_my_ring_pedersen->pub, _other_paillier.get(), &_my_paillier->pub, seed);
    }
    else
    {
...
void batch_response_verifier::process_ring_pedersen(const BIGNUM* e, const mta_range_zkp& proof)
{
    ...
    if (RAND_bytes(reinterpret_cast<uint8_t*>(&gamma[0]), 2 * sizeof(uint64_t)) != 1)
    {
        LOG_ERROR("Failed to get random number, error %lu", ERR_get_error());
        throw_cosigner_exception(cosigner_exception::INTERNAL_ERROR);
    }
    ...
    if (rp_status != RING_PEDERSEN_SUCCESS)
    {
        LOG_ERROR("Failed to init ring pedersen montgomery context, error %d", rp_status);
        throw_cosigner_exception(cosigner_exception::NO_MEM);
    }

    // s^z1*t^z3 == E*S^e
    if (!BN_mod_mul(tmp1, _my_ring_pedersen->lambda, proof.z1, _my_ring_pedersen->phi_n, _ctx.get()) || !BN_mod_add(tmp1, tmp1, proof.z3, _my_ring_pedersen->phi_n, _ctx.get()))
    {
        LOG_ERROR("Failed to calc lambda*x+r, error %lu", ERR_get_error());
        throw_cosigner_exception(cosigner_exception::INTERNAL_ERROR);
    }
    if (!BN_mul_word(tmp1, gamma[0]))
    {
        LOG_ERROR("Failed to calc (lambda*x+r)*random64, error %lu", ERR_get_error());
        throw_cosigner_exception(cosigner_exception::INTERNAL_ERROR);
    }
    ...
    // s^z2*t^z4 == F*T^e
    if (!BN_mod_mul(tmp1, _my_ring_pedersen->lambda, proof.z2, _my_ring_pedersen->phi_n, _ctx.get()) || !BN_mod_add(tmp1, tmp1, proof.z4, _my_ring_pedersen->phi_n, _ctx.get()))
    {
        LOG_ERROR("Failed to calc lambda*x+r, error %lu", ERR_get_error());
        throw_cosigner_exception(cosigner_exception::INTERNAL_ERROR);
    }
...
void single_response_verifier::process_ring_pedersen(const BIGNUM* e, const mta_range_zkp& proof)
{
    ...
    // s^z1*t^z3 == E*S^e
    // tmp1 = z1* lambda + z3
    if (!BN_mod_mul(tmp1, _my_ring_pedersen->lambda, proof.z1, _my_ring_pedersen->phi_n, _ctx.get()) ||
        !BN_mod_add(tmp1, tmp1, proof.z3, _my_ring_pedersen->phi_n, _ctx.get()))
    {
        LOG_ERROR("Failed to calc lambda*z1+z3, error %lu", ERR_get_error());
        throw_cosigner_exception(cosigner_exception::INTERNAL_ERROR);
    }
    ...
    {
        LOG_ERROR("Failed to calc t^(lambda * z1 + z3), error %lu", ERR_get_error());
        throw_cosigner_exception(cosigner_exception::INTERNAL_ERROR);
    }
    ...
    // s^z2*t^z4 == F*T^e
    // tmp1 = z2* lambda + z4
    if (!BN_mod_mul(tmp1, _my_ring_pedersen->lambda, proof.z2, _my_ring_pedersen->phi_n, _ctx.get()) ||
        !BN_mod_add(tmp1, tmp1, proof.z4, _my_ring_pedersen->phi_n, _ctx.get()))
    {
        LOG_ERROR("Failed to calc z2* lambda + z4, error %lu", ERR_get_error());
        throw_cosigner_exception(cosigner_exception::INTERNAL_ERROR);
    }
    ...
    if (!BN_mod_exp_mont(tmp1, _my_ring_pedersen->pub.t, tmp1, _my_ring_pedersen->pub.n, _ctx.get(), _my_ring_pedersen->pub.mont))
    {
        LOG_ERROR("Failed to calc t^(lambda * z2 + z4), error %lu", ERR_get_error());
        throw_cosigner_exception(cosigner_exception::INTERNAL_ERROR);
    }
...
} //fireblocks::common::cosigner::mta
